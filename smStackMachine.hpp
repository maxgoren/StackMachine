...
class StackMachine {
    private:
        ...
        OPSTAT pushr();
        OPSTAT popr();
        vector<string> fetchInstruction(vector<string>&);
        OPSTAT execute();
...
/// @param arg 
/// @return 
OPSTAT StackMachine::push(string argv) {
    if (debugMode) {
        op = "push";
        printTrace();
        printDataStack();
    ...
    data[top++] = atoi(argv.c_str());
    tosr = data[top-1];
    return OP_SUCCESS;
}
/// pushes data onto data stack
OPSTAT StackMachine::push(int argv) {
    if (debugMode) {
        op = "push";
        printTrace();
        printDataStack();
...
vector<string> StackMachine::fetchInstruction(vector<string>& program) {
    return smtokenizer(program, ' ');
}
...
void StackMachine::run(vector<string>& program) {
    ...
    while (programCounter < program.size()) {
        ...
        vector<string> instructions =  fetchInstruction(program); //break the expression
        op = instructions[0];                                   //down into operation and argument
        arg = (instructions.size() > 1) ? instructions[1]:"";
...
void StackMachine::printDataStack() {
    cout<<"Data Stack: [";
...
void StackMachine::printRetAddrStack() {
    cout<<"Return Stack: [";
...
void StackMachine::printTrace() {
    cout<<"["<<programCounter<<"] "<<op<<" "<<arg<<endl;
}

void StackMachine::printDebug() {
    printTrace();
    printDataStack();
    cout<<endl;
    printRetAddrStack();
